    ...
    <div className="space-y-6 md:space-y-10 max-w-7xl mx-auto animate-fade-in pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {statCards.map((stat) => (
          <Button 
            ...
            variant="outline" 
            asChild
            className="h-auto py-4 flex flex-col gap-2 rounded-sm border-white/5 bg-background/40 hover:bg-primary/5 hover:border-primary/30 transition-all group"
          >
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
           {[1,2,3,4].map(i => <div key={i} className="h-32 rounded-sm bg-muted/20 animate-pulse" />)}
        </div>
      ) : (
        <>
          {/* Stats Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {statCards.map((stat, index) => (
              <motion.div
                ...
                className={`p-6 rounded-sm border ${stat.border} ${stat.bg} backdrop-blur-sm relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div className={`p-2.5 rounded-sm bg-background/50 border border-white/5 ${stat.color}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
             <div className="p-6 md:p-8 rounded-sm border border-border bg-card/40 backdrop-blur-sm shadow-sm">
          ...
          {/* Recent Activity */}
          <div className="rounded-sm border border-border bg-card/30 backdrop-blur-md overflow-hidden shadow-md">
            ...
            {recentMedia.length === 0 ? (
              ...
              <div className="divide-y divide-border/40">
                {recentMedia.map((item) => (
                  ...
                  >
                    <div className="w-12 h-12 rounded-sm bg-background/50 border border-border/50 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                    ...
                    <div className="flex items-center gap-3 text-[10px] md:text-xs text-muted-foreground font-mono tabular-nums">
                      <span className="flex items-center gap-1.5 bg-background/50 px-2 py-1 rounded-sm border border-white/5">
                        <Eye className="h-3.5 w-3.5" />
                        {item.view_count}
                      </span>
                      <span className="hidden sm:flex items-center gap-1.5 bg-background/50 px-2 py-1 rounded-sm border border-white/5">
